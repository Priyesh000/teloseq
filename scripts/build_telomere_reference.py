...
def merge_tree(tree: IntervalTree, max_break_point: int = 250) -> IntervalTree:
    ...
    for interval in tree:
        ...
        overlapping_intervals = tree.overlap(
            start - max_break_point, end + max_break_point
        )
...
def adjust_coords(
    reference_length: int, search_region: int, tree: IntervalTree
) -> IntervalTree:
...
def subtelomere_coords(
    tree: IntervalTree, search_region: int, is_q: bool = False
) -> IntervalTree:
...
class Genome:
    ...
    def download_ref(self) -> str:
        ...
        if not self.filename.exists():
            logger.info(f"Please be patient, downloading {self.url} to {self.filename}")
...
class Record:
    ...
    def header(self) -> str:
        ...
        if self.arm == "M":
            return (
                f"{self.chrom}.{self.arm}|{self.genome.genome}|{self.config}:None-None"
            )
        elif self.arm == "Q":
    ...
    def to_tsv(self) -> str:
        ...
        values = [
            self.header,
            self.telomere_coords.begin,
            self.telomere_coords.end,
            self.telomere_coords.length(),
            self.telomere_coords.data.upper(),
            self.subtelomere_coords.begin,
            self.subtelomere_coords.end,
            self.chrom,
            self.arm,
        ]
        if self.arm == "M":
...
def process_stong_etal_assembly(rec: Any, genome: Genome, search_region: int) -> Record:
    ...
    return Record(
        ...
        seq=rec.sequence[:search_region],
        genome=genome,
        telomere_coords=Interval(
            1, 50, data="TTAGGG" if arm.upper() == "Q" else "CCCTAA"
        ),  ## TODO: get the telomere coordinates
...
def process_genome(
    genome: Genome,
    search_region: int = 25_000,
    max_break_point: int = 200,
    hg002_pat: bool = False,
) -> None:
    """Process the genome"""
    ...
    data = []
    for rec in tqdm(
        pysam.FastxFile(genome.filename),
        desc=f"Searching for telomeres: {genome.filename}",
    ):
        # if not rec.name.startswith('chr') or rec.name in ["chrEBV", 'chrM']:

        if genome.genome == "HG002" and hg002_pat:
            if not re.match(
                "(chr[0-9XY]{1,2}_PATERNAL)", rec.name, flags=re.IGNORECASE
            ):
        ...
        adj_qtseq_coord = filter_for_arm(
            adjust_coords(len(rec.sequence), len(qarm), mqtree), is_q=True
        )
...
@click.option(
    "--search_region", "-s", default=25_000, type=int, help="Search region for telomere"
)
@click.option(
    "--max_break_point",
    "-x",
    default=200,
    type=int,
    help="Max break point for merging overlapping intervals",
)
@click.option(
    "--output_dir",
    "-o",
...
@click.option(
    "--hg002_pat", "-p", is_flag=True, help="Output the paternal genome for HG002"
)
@click.option(
    "--hg002", "-hg", 'selected_hg002',  
    is_flag=True, 
    help="Output the paternal and maternal genome for HG002"
)
...
def main(
    ...
    \n
    Example:
    python {__file__} --search_region 25_000 --max_break_point 200 --output_dir refgenome --working_dir workdir my_refgenome
    ...
    with open(output_file, "w") as fasta, open(
        output_file.with_suffix(".tsv"), "w"
    ) as tsv:
        ...
        for genome in genomes:
            for record in tqdm(
                process_genome(
                    genome,
                    search_region=search_region,
                    max_break_point=max_break_point,
                    hg002_pat=hg002_pat,
                )
            ):
